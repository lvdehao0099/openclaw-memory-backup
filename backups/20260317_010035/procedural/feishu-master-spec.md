...
| 热点雷达追踪表 | `VF8IbMNlHaEbNksFG9mcOD6gnAh` | `tblpvRvo1dQ0nSBL` | https://ai-zhineng.feishu.cn/base/VF8IbMNlHaEbNksFG9mcOD6gnAh |
...
### 4.6 表格终极方案：三步法（零空行）✅ 已验证

**核心原理：** 飞书创建表格时会自动在每个单元格生成一个"幽灵块"（空 Text Block），需要 PATCH 覆盖它。

**⚠️ 致命错误：PATCH body 格式**

| ❌ 错误 | ✅ 正确 |
|--------|--------|
| `update_text_elements_request` | `update_text_elements` |

**三步流程：**

```python
# 第一步：创建空表容器
def create_table(doc_id, rows, cols):
...
# 第二步：提取幽灵块 ID
def get_ghost_ids(doc_id, table_id):
    # 获取表格下的所有单元格
    url = f'https://open.feishu.cn/open-apis/docx/v1/documents/{doc_id}/blocks/{table_id}/children'
    ...
    for cell in cells:
        ...
        # 获取单元格下的第一个子块（幽灵块）
        url = f'https://open.feishu.cn/open-apis/docx/v1/documents/{doc_id}/blocks/{cell_id}/children'
        res = requests.get(url, headers=headers)
        if res.json()['data']['items']:
            ghost_ids.append(res.json()['data']['items'][0]['block_id'])
    
    return ghost_ids

# 第三步：PATCH 覆盖幽灵块（关键！）
def update_cell(doc_id, ghost_id, content, bold=False):
    url = f'https://open.feishu.cn/open-apis/docx/v1/documents/{doc_id}/blocks/{ghost_id}'
    
    # 核心：使用 update_text_elements 而不是 update_text_elements_request
    payload = {
        "update_text_elements": {  # ⚠️ 不是 update_text_elements_request！
            "elements": [{
                "text_run": {
...
**完整工具类：** `/root/.openclaw/workspace/scripts/feishu_table_manager.py`

**已验证：**
- ✅ 2026-03-16：创建测试表格，零空行
- ✅ 2026-03-16：创建涌金方案文档，10个表格，全部零空行

---

### 4.6.1 已废弃方案：descendants 参数（不工作）

**注意：** 以下方法在实际 API 中**不工作**，飞书会忽略 descendants 参数。

```python
# ❌ 此方法不工作，descendants 被忽略
