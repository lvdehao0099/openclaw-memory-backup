...
### 0. 启动自检（强制）
```
exec command="~/.openclaw/workspace/scripts/startup-self-check.sh"
```
**自检内容：** 铁律背诵检查、核心文件存在性检查
...
read path="~/.openclaw/workspace/procedural/file-creation-checklist.md"
read path="~/.openclaw/workspace/memory/BUG修复记录.md"
```

### 文件创建铁律（背诵）
❌ **禁止：** 直接写 `write file_path="文件名.md"` → 这会在根目录创建垃圾文件！
✅ **正确：** 必须指定完整路径 `write file_path="/root/.openclaw/workspace/memory/文件名.md"`
✅ **飞书文档：** 必须直接用 `feishu_doc` 创建在「虾米🦞」文件夹

**创建前必须说出：** "我将创建文件：完整路径是 XXX"
