...
async function main() {
    ...
    for (const article of hotData.articles.slice(0, 10)) {
        ...
        const fieldMappings = [
            [['热点标题', '标题', '主题'], article.title],
            [['来源', '发布源'], article.source],
            [['分类', '类别', '类型'], article.category],
            [['相关度评分', '评分', '相关度'], article.relevance_score],
            [['相关度等级', '相关度'], article.relevance_score >= 80 ? "高" : article.relevance_score >= 60 ? "中" : "低"],
            [['选题建议', '建议', '内容方向'], article.topic_suggestion],
            [['摘要', '简介', '描述'], article.summary],
            [['链接', 'URL', '地址'], article.url],
            [['采集日期', '日期'], hotData.collection_date],
            [['过期时间', '有效期至'], expiryStr],
            [['排名', '序号'], article.rank]
        ];
        
        for (const [possibleNames, value] of fieldMappings) {
            const matchedField = possibleNames.find(name => fieldNames.includes(name));
            if (matchedField && value !== undefined) {
        ...
        if (Object.keys(fields).length === 0) {
            // Use the first text field as primary
            const primaryField = fieldNames[0];
            fields[primaryField] = `${article.rank}. ${article.title}`;
            if (fieldNames[1]) fields[fieldNames[1]] = article.summary;
        }
